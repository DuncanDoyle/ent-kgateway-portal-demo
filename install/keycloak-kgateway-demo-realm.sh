...
printf "\nCreating 'admin' Realm Role\n"
CREATE_ADMIN_ROLE_JSON=$(cat <<EOM
{
  "name": "admin",
...
curl -k -X POST -H "Authorization: Bearer ${KEYCLOAK_TOKEN}" -H "Content-Type: application/json" -d "$CREATE_ADMIN_ROLE_JSON" "$KEYCLOAK_URL/admin/realms/$REALM/roles"

# Get the admin role ID
export ADMIN_ROLE_ID=$(curl -k -H "Authorization: Bearer ${KEYCLOAK_TOKEN}" -X GET -H "Accept: application/json" "$KEYCLOAK_URL/admin/realms/$REALM/roles/admin" | jq -r '.id')

# Assign the admin realm role to the admin user
printf "\nAssigning 'admin' role to admin user\n"
ASSIGN_ADMIN_ROLE_JSON=$(cat <<EOM
[{
  "id": "$ADMIN_ROLE_ID",
  "name": "admin"
}]
...
curl -k -X POST -H "Authorization: Bearer ${KEYCLOAK_TOKEN}" -H "Content-Type: application/json" -d "$ASSIGN_ADMIN_ROLE_JSON" "$KEYCLOAK_URL/admin/realms/$REALM/users/$ADMIN_USER_ID/role-mappings/realm"
...
CONFIGURE_PORTAL_CLIENT_JSON=$(cat <<EOM
{
  ...
  "redirectUris": [
    ...
    "http://localhost:7007/kgateway-platform-portal/*", 
    "http://localhost:4000/*", 
    "http://localhost:3000/*"
  ], 
  "webOrigins": ["*"]
...
# Add the groups (realm roles) mapper to the portal-client's dedicated scopes
printf "\nAdding 'groups' realm roles mapper to portal-client\n"
CONFIGURE_GROUPS_CLAIM_IN_JWT_JSON=$(cat <<EOM
[{
  "name": "groups",
  "protocol": "openid-connect",
...
curl -k -X POST -H "Authorization: Bearer ${KEYCLOAK_TOKEN}" -H "Content-Type: application/json" -d "$CONFIGURE_GROUPS_CLAIM_IN_JWT_JSON" $KEYCLOAK_URL/admin/realms/$REALM/clients/${REG_ID}/protocol-mappers/add-models
