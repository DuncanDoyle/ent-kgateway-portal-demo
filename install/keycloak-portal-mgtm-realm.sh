...
export PORTAL_CLIENT_ID=kgateway-portal-idp
...
CREATE_PORTAL_CLIENT_JSON=$(cat <<EOM
{
  ...
  "secret": "kgateway-portal-idp-secret"
}
EOM
)
...
export SERVICE_ACCOUNT_PORTAL_CLIENT_USER_ID=$(curl -k -H "Authorization: Bearer ${KEYCLOAK_TOKEN}" -X GET -H "Accept: application/json" "$KEYCLOAK_URL/admin/realms/$REALM/users?username=service-account-kgateway-portal-idp" | jq -r '.[0].id')
